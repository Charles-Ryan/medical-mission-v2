...
import { ArrowLeft, Trash2, Activity, Heart } from 'lucide-react'
import { toast } from 'sonner'
import { getPatient, removeService, removeCounsel, getServices } from '@/lib/db'
...
export function PatientDetail({ patientId, onBack, onEdit, onDelete }: PatientDetailProps) {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setTempCounselorName(savedCounselor)
  ...
  return (
    <div style={{ padding: 14, background: '#F5FAF5', minHeight: '100%' }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #D8E8D8', borderRadius: 12, marginBottom: 12, overflow: 'hidden', display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 10, padding: '12px 14px' }}>
          <div>
            <label style={{ fontSize: 10, color: '#7A9A7A', textTransform: 'uppercase', letterSpacing: '.05em', display: 'block', marginBottom: 3 }}>Contact</label>
            <span style={{ fontSize: 12, fontWeight: 500, color: '#1C2B1C' }}>{patient.contact_number || '—'}</span>
          </div>
          <div>
            <label style={{ fontSize: 10, color: '#7A9A7A', textTransform: 'uppercase', letterSpacing: '.05em', display: 'block', marginBottom: 3 }}>Gender</label>
            <span style={{ fontSize: 12, fontWeight: 500, color: '#1C2B1C' }}>{patient.gender}</span>
          </div>
          <div>
            <label style={{ fontSize: 10, color: '#7A9A7A', textTransform: 'uppercase', letterSpacing: '.05em', display: 'block', marginBottom: 3 }}>Address</label>
            <span style={{ fontSize: 12, fontWeight: 500, color: '#1C2B1C' }}>{patient.address || '—'}</span>
          </div>
          <div>
            <label style={{ fontSize: 10, color: '#7A9A7A', textTransform: 'uppercase', letterSpacing: '.05em', display: 'block', marginBottom: 3 }}>Medical history</label>
            <span style={{ fontSize: 11, color: '#3D5C3D', fontWeight: 400 }}>{patient.medical_history || '—'}</span>
          </div>
        </div>
